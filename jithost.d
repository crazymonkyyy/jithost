...
void main(string[] args)
{
	...
	writeln("  build-cat [source-dir] [output-dir]	- Build, print index.html, then cleanup");
	writeln("  build-tree [source-dir] [output-dir]	- Build and show directory tree");
	writeln();
	...
	else if (command == "build-cat")
	{
		string sourceDir = (args.length > 2) ? args[2] : "./";
		string outputDir = (args.length > 3) ? args[3] : "./_site";

		writeln("Running build-cat command...");
		import build_tools;
		buildCat(sourceDir, outputDir);
	}
	else if (command == "build-tree")
	{
		string sourceDir = (args.length > 2) ? args[2] : "./";
		string outputDir = (args.length > 3) ? args[3] : "./_site";

		writeln("Running build-tree command...");
		import build_tools;
		buildTree(sourceDir, outputDir);
	}
	else
	{
